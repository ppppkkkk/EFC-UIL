...
class Struc2Vec():
    def __init__(self, graph, walk_length=10, num_walks=100, workers=1, verbose=0, stay_prob=0.3, opt1_reduce_len=True, opt2_reduce_sim_calc=True, opt3_num_layers=4, temp_path='./temp_struc2vec/', reuse=False):
        self.graph = graph
        ...
        print('self.create_context_graph...(workers=:%d)'%workers)
        self.create_context_graph(self.opt3_num_layers, workers, verbose)
        print('self.prepare_biased_walk...(workers=:1)')
        self.prepare_biased_walk()
        print('BiasedWalker...(workers=:1)')
        self.walker = BiasedWalker(self.idx2node, self.temp_path)
        print('self.walker.simulate_walks...(workers=:%d)'%workers)
    ...
    def train(self, embed_size=128, window_size=5, iter=5):

        # pd.read_pickle(self.temp_path+'walks.pkl')
        sentences = self.sentences
        
        print("Learning representation...(workers=%d)"%self.workers)
        model = Word2Vec(sentences, size=embed_size, window=window_size, min_count=0, hs=1, sg=1, workers=self.workers,
                         iter=iter)
    ...
    def _get_order_degreelist_node(self, root, max_num_layers=None):
        #print('Opt3: max_num_layers=%d'%max_num_layers)
        if max_num_layers is None:
            max_num_layers = 8 #float('inf') #8层已经很高了
